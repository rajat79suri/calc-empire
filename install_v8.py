 ...
 * Handles Loan, Growth, and Amortization logic.
 */
...
export const calculateLoan = (P, r, n, extraMonthly = 0) => {
  ...
  // Simulation Loop (Limit to 100 years to prevent crashes)
...
export const getAmortizationData = (balance, rate, monthlyPayment, startYear) => {
    ...
    for (let i = 1; i <= 3600; i++) { 
       ...
       if (i % 12 === 0 || currentBalance <= 0) {
          ...
          yearTotalInterest = 0; yearTotalPrincipal = 0; currentYear++;
       }
    }
    return data;
};
...
// SSR Defaults for SEO
let initialResult = "0";
let initialPayoff = "Dec, 2055"; 
if (logic_type === 'loan') { initialResult = "2,385.42"; } 
else if (logic_type === 'growth') { initialResult = "100,000.00"; }
---
  ...
  <div class="lg:col-span-7 flex flex-col gap-6 print:col-span-12 print:w-full">
    ...
    <div class="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm print:bg-white print:border-black">
      ...
      <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex gap-3 print:hidden">
        ...
        <button id="print-btn" class="flex-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 py-2 rounded-lg text-sm font-bold hover:bg-gray-50 transition-colors flex justify-center items-center gap-2">
           🖨️ Save PDF
        </button>
      </div>
    </div>
...
# WRITE FILES
with open(os.path.join(lib_dir, "financial-math.js"), "w") as f:
    f.write(math_lib_code)

with open(os.path.join(comp_dir, "CalculatorForm.astro"), "w") as f:
    f.write(component_code)

# CLEANUP
if os.path.exists("install_engine.py"):
    os.remove("install_engine.py")

print("✅ V8 UPGRADE COMPLETE: Math Library and UI updated.")
```
...
```bash
python install_v8.py
```

#### Step 3: Launch
If you see the success message, launch the V8 Connectivity Suite.

```bash
npm run build
git add .
git commit -m "V8 Upgrade: Connectivity Suite"
git push